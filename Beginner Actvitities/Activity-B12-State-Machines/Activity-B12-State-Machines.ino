/* ================================================================================
Beginner Activity 12: State Machines [Activity-B12-State-Machines]
April 14, 2026
...
1.  A state machine can be described visually as a state diagram:
    ...
    checked continuously on every loop iteration. Why is this cleaner?
    What would happen if a noisy connection briefly triggered
    'enter_state()' a second time while already in a state?
...
5.  The 'enter_state()' function prints a diagnostic message
  ...
  --> GREEN (advanced green complete)
    ...
    Compare this to printing every loop iteration. How many lines
    per second would be printed at LOOP_DELAY = 1ms if every
    iteration printed a status message? Why would that be less
    useful than transition-only printing?
...
2.  Crosswalk extension: the main program already processes a
    ...
    b) Walk request during green: currently a walk request pressed
       ...
       immediately. Guard against extending an already-extended phase.

    For part (b), how do you extend the current green phase without
    restarting the state timer? Consider using 'effective_green'
    and 'walk_requested' together to calculate the remaining time.

3.  Combination lock: implement a three-button combination lock
    as a standalone state machine.
    Open: ARPS2-Project-Combination-Lock
