...
float read_temp_c()
{
    ...
    // Formula for MCP9700/MCP9700A: Temp(C) = (Voltage - 0.5V) / 0.01 V/C
    // Formula for TMP-36:           Temp(C) = (Voltage - 0.5V) / 0.01 + 25
    float voltage = (float)analogRead(U4) * 3.3 / 1023.0;
    return (voltage - 0.5) / 0.01;         // Use this line for MCP9700/MCP9700A
    // return (voltage - 0.5) / 0.01 + 25; // Use this line for TMP-36
}
...
1.  The Arduino UNO's ADC returns values from 0 to 1023 -
...
6.  The 'read_temp_c()' function converts the raw ADC reading to
    degrees Celsius using the characteristics of the analog
    temperature sensor fitted to your ARPS-2 board. The ARPS-2
    can be assembled with either an MCP9700/MCP9700A or a TMP-36
    sensor - both output 0.5V at a reference temperature and change
    at 10mV per degree Celsius, but their reference temperatures
    differ, which changes the conversion formula.

    The conversion always begins by turning the raw ADC value into
    a voltage:
    ...
    The MCP9700/MCP9700A outputs 0.5V at 0°C and increases by
    10mV (0.01V) per degree Celsius, giving:

    Temp(C) = (Voltage - 0.5) / 0.01

    The TMP-36 also outputs 0.5V and changes at 10mV/°C, but its
    0.5V reference point is 25°C rather than 0°C, giving:

    Temp(C) = (Voltage - 0.5) / 0.01 + 25

    Why does a different reference temperature require adding 25
    to the result? If both sensors change at the same 10mV/°C
    rate, what is the only difference between their outputs at
    any given temperature?

    Check which sensor is fitted to your board and enable the
    correct return statement in 'read_temp_c()'. At what raw ADC
    value would each sensor read exactly 0°C? Calculate both and
    verify the one that applies to your board by adding a temporary
    Serial.print inside 'read_temp_c()'.
...
2.  Write a 'temp_trend()' function that compares the current
    ...
    ARPS-2 uses U4 as its only temperature sensor - how
