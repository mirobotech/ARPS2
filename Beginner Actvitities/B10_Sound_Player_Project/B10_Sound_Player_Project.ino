...
This project controls the ARPS-2's piezo speaker using Arduino's
tone() function. Frequency (pitch) is fully controllable with SW2
and SW5. Arduino's tone() always produces a 50% duty cycle square
wave - timbre control is not available through tone(). See EA4
if you are interested in how timbre could be explored using
...
--------------------------------------------------------------------------------
EA 4 - Duty cycle and direct timer access
--------------------------------------------------------------------------------
...
Research your Arduino UNO board's hardware timers and how
'tone()' uses them internally. Producing an independent duty
cycle at an arbitrary frequency requires configuring a timer
directly, bypassing both 'tone()' and 'analogWrite()' - a more
advanced technique than this project covers, but worth knowing
exists if you want full control over both pitch and timbre.
