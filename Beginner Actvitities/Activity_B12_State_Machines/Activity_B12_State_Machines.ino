...
NOTE: This program uses all four ARPS-2 pushbuttons (SW2-SW5) as
independent keys, with none left over to give up - it assumes an
Arduino UNO R4 (Minima or WiFi), since SW2 and SW3 are shared with
the USB serial port on Arduino UNO Rev 3.
...
void setup()
{
    ...
    // NOTE: this program uses all four buttons as independent keys -
    // see the header note above about Arduino UNO R4 vs. Rev 3.
