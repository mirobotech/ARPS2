...
This activity's optional light-sensing feature uses the left optical
sensor module in floor-sensing mode (phototransistor Q1 installed
alone) and the right module with its phototransistor Q3 installed,
the same repurposed light-sensor arrangement used in Activity 9.
=================================================================================*/
// IMPORTANT: Make sure that ARPS2.h is accessible as a tab in this project.
#include "ARPS2.h"      // Define ARPS-2 I/O devices

// ---- Program Configuration ------------
const bool LIGHT_SENSORS_INSTALLED = false;  // Set true if Q1 and Q3 are installed
...
void setup()
{
    ...
    if (LIGHT_SENSORS_INSTALLED)
        Serial.println("Q1/Q3: control LED4/LED5 brightness");
    Serial.println();
}
...
void loop()
{
    ...
    // Q1 and Q3 control LED4 and LED5 brightness directly (if installed) -
    // brighter ambient light makes the corresponding LED glow brighter
    if (LIGHT_SENSORS_INSTALLED)
    {
        set_brightness(LED4, Q1_level() / 4);
        set_brightness(LED5, Q3_level() / 4);
    }
...
Open ARPS2.h and find where 'LED2' is defined. It is a
...
a pin for analog output. The classic Arduino UNO's ATmega328P chip
supports hardware PWM on pins 3, 5, 6, 9, 10, and 11 - notice that
all four of ARPS-2's LED pins (LED2=3, LED3=9, LED4=10, LED5=11)
fall within this set, which is why 'analogWrite()' works on every
LED without needing to check pin compatibility first. Which of
ARPS-2's other pins, if any, could also be used with 'analogWrite()'?
...
The default PWM frequency on the classic Arduino UNO is approximately
490 Hz on pins 3, 9, 10, and 11 - the pins used by all four of
ARPS-2's LEDs - which appears steady to the eye for LED dimming. To
observe the effect of PWM switching directly, try waving a
PWM-dimmed LED quickly through the air in a darkened room - at a
lower duty cycle you should see a dotted trail rather than a solid
line, which reveals the rapid on-off switching.

Why do motor controllers typically use PWM frequencies of
10 kHz or higher even though motors have no visual response?
Hint: what might you hear if a motor were driven at 500 Hz? At
approximately 490 Hz, is this something you might actually be able
to hear from one of ARPS-2's own motors?
...
The 'LIGHT_SENSORS_INSTALLED' constant is a configuration flag - a
...
If LIGHT_SENSORS_INSTALLED is false, what happens to LED4 and LED5?
Their pins are still configured as outputs in setup(). What
is their state when the LIGHT_SENSORS_INSTALLED block never runs?

When LIGHT_SENSORS_INSTALLED is true, 'Q1_level()' returns 0-1023
(this program never calls 'analogReadResolution()', so it stays at
Arduino's default 10-bit range - see Activity 9, GE1, for why ARPS-2
stays at 10-bit rather than matching the BEAPER Nano version's
16-bit range), but 'analogWrite()' accepts 0-255. The program scales
the reading with a simple integer division:

Example code:

set_brightness(LED4, Q1_level() / 4);

Why does dividing by 4 work here, rather than the division by 256
used in the BEAPER Nano version of this activity? What is the
maximum value of 'Q1_level() / 4', and does it fit within the
0-255 range? When would 'map()' be needed instead of a simple
division?
...
that happen in one loop() iteration with LIGHT_SENSORS_INSTALLED = true
