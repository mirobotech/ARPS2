/* ================================================================================
Beginner Activity 10: Analog Output [ARPS2-Activity-B10-Analog-Output]
March 2, 2026

Platform: mirobo.tech ARPS-2 circuit
Requires: ARPS2.h header file

Optional analog input:
  An external potentiometer can be connected to header H1 or H4
  and read using analogRead() to control LED brightness directly.
  See GE6 and the ARPS-2 schematic for wiring details.
=================================================================================*/
...
// NOTE: analogWrite() uses 0-255 (8-bit). PWM frequency is set by the hardware
// timer and cannot be changed in software on Arduino - see GE3.
...
void setup()
{
    ...
    // Configure ARPS-2 LEDs as PWM-capable outputs.
    // analogWrite() works directly on any PWM-capable pin after
    // pinMode() - no special object creation is needed.
    pinMode(LED2, OUTPUT);
    pinMode(LED3, OUTPUT);
    pinMode(LED4, OUTPUT);
    pinMode(LED5, OUTPUT);
...
void loop()
{
    ...
    // An external potentiometer connected to H1 or H4 can control
    // LED4 or LED5 brightness directly - see GE6.
    // set_brightness(LED4, analogRead(AH1) / 4);   // Example: pot on H1
    // set_brightness(LED5, analogRead(AH4) / 4);   // Example: pot on H4
...
2.  In Arduino, 'analogWrite()' works directly on any PWM-capable pin
    ...
    a pin for analog output. Which other pins in ARPS2.h could
    be used with 'analogWrite()'? Look for pins that are on
    PWM-capable hardware in the header comments or in the Arduino
    UNO R4 datasheet (or Arduino UNO Rev. 3).

3.  Arduino's 'analogWrite()' uses a fixed PWM frequency set by the
    hardware timer for each pin.

    The default PWM frequency on the Arduino UNO is typically
    490 Hz on most pins and 980 Hz on pins 5 and 6, which appears
    steady to the eye for LED dimming. To observe the effect of PWM
    switching directly, try waving a PWM-dimmed LED quickly through
    the air in a darkened room - at a lower duty cycle you should
    see a dotted trail rather than a solid line, which reveals the
...
6.  The ARPS-2 circuit does not include built-in potentiometers,
    but an external potentiometer can be connected to header H1
    or H4 and read using 'analogRead()'. The commented-out lines
    in 'loop()' show how this would work:

  set_brightness(LED4, analogRead(AH1) / 4);

    'analogRead()' returns a 10-bit value (0-1023) while
    'analogWrite()' accepts an 8-bit value (0-255). Why is
    dividing by 4 the correct scaling factor? What would happen
    if you used '/ 2' or '/ 8' instead - what brightness range
    would result in each case?

    The ADC (10-bit, 0-1023) and PWM (8-bit, 0-255) use different
    resolutions because they serve different purposes - the ADC
    measures voltage accurately while PWM controls perceived
    brightness. These independent design decisions require an
    explicit scaling step when one feeds the other.

    If you have an external potentiometer available, connect it
    to H1 following the ARPS-2 schematic, uncomment the relevant
    line in 'loop()', and rotate the pot through its full range.
...
7.  This program runs in a continuous loop() function, checking all
    ...
    that happen in one loop() iteration with all buttons released.
    How many iterations per second does the program run at
    STEP_DELAY = 20ms?
...
Extension Activities
    ...
    output - sound, motor control, or servo control. Each begins in
    a separate project file that sets up the hardware configuration
