...
Write a program that waits in a loop for SW4 to be pressed, then
waits in a second loop for SW4 to be released, then lights LED2 to
...
of times equal to the number of times SW4 was pressed before SW5 was
pressed. The program should:

 - Wait in a loop for button presses, counting SW4 presses using edge
...
Create a reaction timer program. The program should:
 ...
 - Wait in a loop for SW4 to be pressed, using a fixed delay(10)
...
B06_Combination_Lock_Project is a separate, open-ended project
file that asks you to build a button-sequence lock, using a step
counter and conditional loops from this activity's examples. The
project is provided as a skeleton with TODOs, making it a good next
step to build confidence and design projects yourself.
