...
4.  Arduino's tone() function always produces a 50% duty cycle
    ...
    On the Arduino UNO, direct timer register access can set both
    frequency and duty cycle independently, but this requires
    working with AVR hardware registers beyond the standard Arduino
    API. Research 'Arduino UNO Timer1 PWM' or 'ICR1 OCR1A' for
    examples. What timbre change do you expect to hear when the
    duty cycle is very low (say, 5%)?
