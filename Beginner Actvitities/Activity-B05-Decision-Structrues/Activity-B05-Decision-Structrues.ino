/* ================================================================================
Beginner Activity 5: Decision Structures [Activity-B05-Decision-Structures]
February 26, 2026
...
if/else if chain - a way of expressing multi-way decisions - along
with comparison operators, counting variables, and edge detection.
These tools, combined with the named variables and Boolean logic from
...
1.  This program's button variables (SW3_pressed, SW4_pressed, etc.)
    ...
    combined using the Boolean logical operators '&&' (AND), '||'
    (OR), and '!' (NOT). For example, changing the if condition in
    the program to:
...
2.  Replace the '&&' operator in the if condition with '||' and run
...
3.  It's just as easy to invert an input to its opposite state using
    the '!' (NOT) logical operator. Predict how you think this
    conditional expression will behave:
...
4.  Let's make the program count the number of times SW3 is pressed.
    ...
    of SW3. Next, add the 'SW3_presses++' increment operation inside
    ...
    Add Serial.print() calls inside the if block to display the count
    in the Serial Monitor each time SW3 is pressed:
    ...
    Run the program, open the Serial Monitor, and press SW3 a few
    times. How quickly does the count increase? Does the count go up
    once per press, or faster? What do you think is happening?

5.  Previous programs have used the '==' (equal to) conditional
    operator to check if two values are the same. Arduino C supports
    ...
    Now add this set of if/else if conditions below the SW3 if block
    to display the count using the LEDs:
    ...
    Starting with the first if, each condition is evaluated in turn
    until the first true condition is found. Once a true condition is
    found, its code runs and the rest of the chain is skipped entirely.
...
6.  You may have noticed that the counter increases very quickly -
    ...
    Add both of these changes to your program. The Serial.print()
    calls from GE4 will now show exactly one count per press in the
    Serial Monitor, regardless of how long the button is held. This
    is a good example of using serial output to verify that a fix is
    working correctly - once you are satisfied, the Serial.print()
    calls can be removed or commented out.

    The SW4_last variable is also already declared for the same
    purpose. Can you add edge detection for SW4 as well?

7.  Remove the Serial.print() calls from your program and run it to
    confirm everything still works correctly. Now think back to the
    program in Activity 4's Exploration file, where pressing a button
    during the light pattern had no effect until the pattern finished.
    How is the approach in this activity different, and what makes
    edge detection better suited to counting presses accurately?
...
constants throughout, and use Serial.print() to help develop and
...
bool game_active = true;
    ...
    While developing your game, use Serial.print() to track both
    counters at once:

    Serial.print("P1: ");
    Serial.print(SW3_presses);
    Serial.print("  P2: ");
    Serial.println(SW4_presses);

    Once your game is working correctly, remove or comment out the
    Serial.print() calls - debugging output left in a finished program
    clutters the Serial Monitor and can be confusing.
