...
loop delay. Each control now has its own update rate:
  ...
  LED4/5 rate:    how often an external pot reading updates LED brightness
...
Controls (same as Activity 10):
  ...
  H1 pot  - control LED4 brightness (if external pot connected)
================================================================================*/
...
void loop()
{
    ...
    // ---- Pot polling timer (external pot on H1, if connected) ----
    // TODO: check if POT_RATE ms has elapsed since last_pot_time
    //       if so: read analogRead(AH1), scale to 0-255, update LED4
    //              update last_pot_time = current_time
    //       analogWrite(LED4, map(analogRead(AH1), 0, 1023, 0, 255));
