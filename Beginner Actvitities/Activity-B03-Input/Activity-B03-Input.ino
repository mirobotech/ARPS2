...
It also helps to know that ARPS-2's pushbuttons are wired as
...
1.  The pushbuttons in the ARPS-2 circuit are connected in what is
    known as a pull-up configuration, meaning one side of each
...
4.  Imagine that you're creating a turn signal circuit for a bicycle.
    ...
    using one or more of ARPS-2's LEDs to indicate a left or right
    turn while the corresponding button is held.
