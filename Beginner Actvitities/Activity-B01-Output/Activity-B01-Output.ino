/* ================================================================================
Beginner Activity 1: Output [ARPS2-Activity-B01-Output]
March 30, 2026
...
2.  The first real statement in this program includes a 'header file'
    for your ARPS-2 circuit:

#include "ARPS2.h"

    The 'ARPS2.h' header file must be accessible as a tab in the
    ...
    configures the ARPS-2's I/O devices and microcontroller pins for
    you, helping you to start writing programs for it more quickly.
    (You can click the ARPS2.h tab in the IDE to see the code it
    contains!)

    This include statement makes the contents of ARPS2.h available
    ...
    constants defined in ARPS2.h. The pinMode() statement configures
    the LED2 pin as an OUTPUT, and the digitalWrite() statement sets
    its output voltage to either HIGH (5V) or LOW (0V). Setting it
    HIGH turns LED2 on!

    All of the ARPS-2's other LEDs (LED3 - LED5) can be controlled
...
4.  The ARPS2.h header file refers to the pin definition for the
    ...
    The Arduino UNO's on-board LED is called LED_BUILTIN. To use it,
    first configure it as an output, then turn it on using
    digitalWrite(), exactly as with the ARPS-2's other LEDs:
    ...
    (The LED_BUILTIN indicator isn't ideal since it's hidden under
    the ARPS-2 circuit, but you should be able to see it glowing
    through the board.)
