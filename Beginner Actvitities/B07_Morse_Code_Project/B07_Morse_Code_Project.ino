/* ================================================================================
Beginner Activity 7 Project: Morse Code [B07-Morse-Code-Project]
Version: 1.2
...
Morse code represents letters as sequences of short and long signals
- dots and dashes. This project asks you to encode a message of your
own choosing, using a for loop to step through a sequence of symbols,
the same way GE 4 previewed iterating directly over a string.
...
Your Morse code project should:
 ...
 - Leave a gap between symbols within a letter, a longer gap between
   ...
   real Morse code uses a dash three times as long as a dot, a gap
   between letters three times the dot duration, and a gap between
   words seven times the dot duration - but you can choose your own
   timing if you prefer.)
...
your chosen message. Look up the Morse code patterns for the letters
...
Stretch goals:
 ...
 - Encode the full alphabet using an array of structs (or two
   parallel arrays) mapping each letter to its dot/dash pattern, so
   any short message could be converted automatically.
 ...
 - Display the message being sent in the Serial Monitor, one letter
   at a time, in sync with the LED and tone output.
...
// letters - that's how SOS is actually sent in real Morse code. In
// C, this should be a 'const char*' - a pointer to the pattern text -
// since Part 2 will need to point it at different patterns as the
// message is encoded.
...
void setup()
{
    ...
    stopping when you reach the '\0' that marks the end of the string
    (see this activity's GE 4). For each symbol:
    ...
    */
    ...
    const char* MESSAGE = "SOS";, or a word of your choice).

    TODO: Wrap your Part 1 code in an outer for loop that iterates
    over each letter in MESSAGE (also stopping at '\0'). At the start
    of each iteration, before your existing Part 1 loop runs, set
    'pattern' using an if/else if chain that compares the current
    letter against each pattern you need - you only need the letters
    that appear in your own message, not the whole alphabet. Look up
    the Morse code patterns for those letters before you start.
    ...
    instead of once total, using whichever pattern the if/else if
    chain just looked up.

    TODO: Pause for the letter gap after each letter's pattern
    finishes, before moving to the next letter. Between words, if
    your message contains more than one, pause for the (longer) word
    gap instead.
    */
}
...
void loop()
{
    // This project runs once in setup(). See Activity 6, GE 4 for a
    // reminder of how to restructure a program to repeat using loop().
}
