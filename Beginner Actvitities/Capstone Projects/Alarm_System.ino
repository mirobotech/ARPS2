/* ================================================================================
Capstone Project: Alarm System [ARPS2-Capstone-Alarm-System]
Version: 1.0
...
buttons are genuinely needed as code digits (the same situation as
the Activity 12 capstone combination lock and safe).
...
Sensor options (choose one, or combine - see EA3):
  Option A - Optical proximity sensor (contactless):
    ...
    floor/line sensors instead: Q1, Q2, or Q3. These detect a trip
    by reflectivity, as an object or hand approaches the sensor, the
    same way they detect a floor or line when used on the robot
    chassis. Mount the sensor facing the door or opening. Set
    SENSOR_MODE to SENSOR_OPTICAL and calibrate SENSOR_THRESHOLD for
    your setup - see take_sensor_baseline() and Step 1 in the
  ...
  Option B - Contact switch on an expansion header:
    ...
    wire a normally-closed (NC) switch to a spare header (H1, H3, or
    H4 - H2 is shared with LED_BUILTIN) and GND. Set SENSOR_MODE to
    SENSOR_CONTACT. A mechanical switch is more likely to be
    available in a school's parts bin than a reed switch, but its
    contacts can bounce - see Activity 11 EA1's debounce pattern if
    'check_sensors()' behaves inconsistently right at the moment of
    a trip.
...
const int SENSOR_THRESHOLD = 100;             // Optical: reflectivity change that indicates a trip

// Only used if SENSOR_MODE is SENSOR_CONTACT. Change if you wire the
// switch to a different header (H2 is shared with LED_BUILTIN - avoid it).
...
bool check_sensors()
{
    ...
    {
        // TODO: compare Q1_level() to sensor_baseline. Both a
        //       decrease and an increase in reflectivity can
        //       indicate a trip, so compare the absolute
        //       difference: abs(current - sensor_baseline) >
        //       SENSOR_THRESHOLD. Return true or false based on this
        //       comparison. If you are using Q2 or Q3 instead of Q1
        //       (see the header comment), call that function here
        //       instead.
        return false;
    }
...
update SENSOR_SWITCH_PIN if so, avoiding H2) and GND. Verify that
...
Extend 'check_sensors()' to read two of Q1, Q2, and Q3 (for example,
one facing each of two entry points), returning true if either is
triggered. Print which sensor tripped to the Serial Monitor for
debugging.
