/* ================================================================================
Capstone Project: Combination Safe [ARPS2-Capstone-Combination-Safe]
Version: 1.0
...
This capstone assumes an Arduino UNO R4 (Minima or WiFi), which
frees SW2 and SW3 for use alongside SW4 and SW5 - unlike other
ARPS-2 activities in this curriculum, this project does not work
around the Arduino UNO Rev 3 USB-serial pin sharing, since all four
buttons are genuinely needed as code digits.
...
Hardware used:
  ...
  Servo on H9 - Locking mechanism, using the Arduino Servo library

NOTE ON HEADER PINS: H9 and H10 are dedicated servo/digital output
pins on ARPS-2, not shared with H1-H4 - so there is no pin conflict
between your servo choice and your door-sensor header choice here.
There is a different consideration, though: H2 is shared with
LED_BUILTIN (the status LED used throughout this curriculum), so
this project defaults to H1, H3, or H4 for the door sensor's contact
switch rather than H2.

Door sensor options (choose one):
  Option A - Optical proximity sensor (contactless):
    ARPS-2 has no ambient light sensor - use one of the break-away
    floor/line sensors instead: Q1, Q2, or Q3. These detect the door
    by reflectivity, as a surface on the door approaches the sensor,
    the same way they detect a floor or line when used on the robot
    chassis. Mount the sensor so it faces the closing door. Set
    SENSOR_MODE to SENSOR_OPTICAL and calibrate SENSOR_THRESHOLD for
    your setup - see take_sensor_baseline() and Step 3 in the
    Development Guide below. Change 'Q1_level()' to 'Q2_level()' or
    'Q3_level()' in both functions if you use a different sensor.
  ...
  Option B - Contact switch on an expansion header:
    ...
    wire it to H1, H3, or H4 (see the pin-sharing note above) and
...
const int SENSOR_THRESHOLD     = 100;              // Optical: reflectivity change that indicates door closed
...
// switch to a different header (see the pin-sharing note above).
const int DOOR_SWITCH_PIN = H1;
...
Servo lockServo;                     // Attached to H9 in setup() - change to H10 if needed
...
int  sensor_baseline    = 0;         // Optical mode: sensor reading with the door open
...
void take_sensor_baseline()
{
    ...
    // Change Q1_level() to Q2_level() or Q3_level() here too if you
    // are using a different floor/line sensor.
    sensor_baseline = Q1_level();
...
bool door_is_closed()
{
    ...
    {
        // TODO: compare Q1_level() to sensor_baseline. A change in
        //       reflectivity large enough to exceed SENSOR_THRESHOLD
        //       means the door has closed and is now in front of
        //       the sensor. Return true or false based on this
        //       comparison. If you are using Q2 or Q3 instead of
        //       Q1 (see the header comment), call that function
        //       here instead - the rest of the comparison works
        //       the same way.
        return false;
    }
...
void setup()
{
    ...
    // Configure ARPS-2 pushbuttons as inputs. This capstone assumes
    // an Arduino UNO R4, so all four buttons are used without the
    // Rev 3 USB-serial pin-sharing workaround other ARPS-2 activities
    // in this curriculum apply.
    ...
    all_leds_off();
    lockServo.attach(H9);                // Change to H10 if needed
...
optical mode, print 'Q1_level()' (or Q2/Q3, whichever you chose)
while opening and closing the door to find a SENSOR_THRESHOLD that
reliably distinguishes the two. For contact mode, test that
'digitalRead(DOOR_SWITCH_PIN)' returns the expected reading for
both open and closed. Verify 'door_is_closed()' returns true only
when the door is actually closed, in both directions of travel.
