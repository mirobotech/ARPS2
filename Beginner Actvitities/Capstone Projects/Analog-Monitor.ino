/* ================================================================================
Capstone Project: Analog Sensor Monitor [ARPS2-Capstone-Analog-Monitor]
April 22, 2026
...
On-board analog inputs available:
    temp_level()    - Analog temperature sensor U1 (A2)
    ADIV_level()    - Voltage divider tap (A3) - useful for battery monitoring

External analog inputs available:
    AH1 (A4)  - Sensor header H1 with analog input, 5V, GND
    AH4 (A5)  - Sensor header H4 with analog input, 5V, GND

Digital inputs available:
    SW2-SW5         - Pushbuttons (active LOW, INPUT_PULLUP)
    H1-H4, H9, H10  - Expansion headers (for external sensors/switches)

Analog outputs available:
    LED2-LED5       - LEDs (on/off or PWM brightness via analogWrite)
    LS1             - Piezo speaker (tone() / noTone())
    H9, H10         - Servo position (Servo library)

Digital outputs available:
    LED2-LED5       - LEDs (on/off)
    H1-H4, H9, H10  - Digital output headers
...
// Uses right_motor_forward() / right_motor_stop() from ARPS2.h motor functions.
