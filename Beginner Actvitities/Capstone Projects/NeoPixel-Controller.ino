...
Hardware used:
  ...
  LED_BUILTIN - On while a mode is active
              (LED_BUILTIN = pin 13 = SONAR TRIG. This sketch does not
              use SONAR, so LED_BUILTIN is available. If SONAR is added
              later, remove all LED_BUILTIN references.)
...
WARNING: A 60 LED strip at full brightness draws up to 3.6A (60ma
  ...
  the highest expected current. Connect the ARPS-2 GND to the
  external power supply GND (shared ground), and run the data
  wire from the ARPS-2 to the Din pin on the strip.

* Connecting short NeoPixel sticks, rings, or strips *

WS2812B or SK6812 LEDs:
  Power the ARPS-2 with an external power supply (7-12V) connected
  to the Arduino UNO power jack. Up to 30 LEDs can be connected to
  H9 (pin 7) or H10 (pin 8) at low brightness (MAX_BRIGHTNESS = 32
  or less). Both WS2812B and SK6812 strips work with the 5V digital
  output of the ARPS-2.
...
const int STRIP_TYPE = 1;         // 0 = RGB (WS2812B),  1 = RGBW (SK6812)

// Data pin: H9 (pin 7) or H10 (pin 8).
// Both pins are independent digital I/O with no shared peripherals.
// H9 is the default - change to H10 if H9 is needed for another purpose.
// Both WS2812B and SK6812 strips work with the 5V digital output of H9.
const int DATA_PIN = H9;
...
Step 1 - Hardware setup and first pixel
  Wire the strip: data input to DATA_PIN, strip GND to ARPS-2 GND,
  ...
  and set STRIP_TYPE = 1 before testing. The data pin remains H9 - the
  ARPS-2 5V digital output drives SK6812 strips correctly.
...
Step 10 - RGBW strips (SK6812)
  ...
  3. The data pin does not change: H9 works for both WS2812B and SK6812
     on the ARPS-2. The Arduino UNO R4's 5V GPIO output drives both strip
     types correctly - no separate 3.3V connection is needed.
...
Extensions
  ...
  d) Beat-reactive brightness: read an analog input (e.g. ADIV_level()
     for battery voltage monitoring, or an external microphone or light
     sensor wired to AH1) using analogRead() and map the reading to
     MAX_BRIGHTNESS. Which mode looks best with a changing brightness?
