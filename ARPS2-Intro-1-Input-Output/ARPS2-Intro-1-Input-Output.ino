...
1.  Let's analyze this program! Program analysis is an important skill to
    ...
    your ARPS2 circuit on top of your Arduino, and then connect the Arduino
    to your computer. Press the Upload button at the top of the IDE screen --
    it's the large right-facing arrow. The IDE will start compiling the
    program sketch and will then upload the compiled program to the Arduino.
    Press and release pushbutton SW2 on your ARPS2 circuit while observing
    its LEDs.
    ...
    SW2 is pressed, and why you think flashing pattern continues -- instead
    of stopping immediately -- after pushbutton SW2 is released.
...
2.  The first line of code in the program's main 'loop()' function is:
  ...
  SW2State = digitalRead(SW2);
    ...
    defined as SW2, and stores the voltage as one of two possible states
    states in the SW2State variable. If the voltage on the pin is high,
    SW2State will become HIGH, and if the voltage is low SW2State will
    be LOW.
  ...
  if(SW2State == LOW) {
    ...
    LED2 high, enabling current to flow through LED D2 in the circuit.
    ...
    What real-world voltages do you think the values HIGH and LOW correspond
    to on the Arduino UNO used with your ARPS2?
...
4.  Understanding circuit operation is just as important as understanding
    program operation when analyzing and debugging interface circuits
    (hardware components interacting with or controlled by software). If
    ...
    verify the HIGH or LOW values you predicted above. To be able to read
    the LED voltages, you may need to lengthen the time delay after the
    output statement for the LED being measured.

    First, try measuring the potential across one of the LEDs. Press SW2
    and note how the voltage changes. Is the voltage what you expected it
    to be? If not, use the schematic to look up the resistor connected to
    the LED you were measuring, and measure the potential across both the
    resistor and LED. Is this closer to the voltage you expected?
    ...
    a change in the voltage across the pushbutton. Does this value match
    with your prediction? How does this value relate to the value used by
    the SW2 input statement?
...
5.  Let's compare the operation of 'if' and 'while' structures by making two
    ...
    between the existing SW2 'if' block and the 10ms sleep delay, shown by the
    location of the comment in the program code.
  ...
  digitalWrite(LED5,LOW);
    ...
    Next, press and release pushbutton SW5. LED D5 should operate exactly
    the same way as LED D4, staying lit only while SW5 is being pressed.
    (In TinkerCad Shift-clicking the first button makes it stay pressed
    so you can click on a second button.)
    
    While both program switch blocks are designed to effectively mimic the
    operation of real-world momentary pushbuttons, implementing these using
    a sequence of software steps makes them behave differently than physical
    physical buttons would. To see the difference press and hold SW4, and
    while holding SW4 press and release SW5. Do both switches still operate
    as expected?
    
    Now, try it in the opposite order. Press and hold SW5, and then press
    and release SW4. Do both switches operate as expected?
    
    Explain the differences in the operation of the 'if' and 'while' blocks
    used to make the momentary button functions. How does each affect the
    program flow -- the steps taken by the microcontroller through the 
    main program loop. Which block should be used to sense both switches
    simultaneously, as if they were wired in parallel?

6.  Comment-out the second 'SW5State = digitalRead(SW5);' statement -- the
    one inside the while loop -- by adding two slashes '//' at the
    beginning of the line, like this: '//SW5State = digitalRead(SW5);'.
    Re-upload your program and test SW5 again. After that, try testing
    SW4 and SW5. Do they still work? If the program seems to be getting
    stuck, press and release the SW1 RESET pushbutton on ARSP to reset the
    microcontroller and re-start the program. Explain what you think is
...
7.  Next, let's try nesting if conditions to mimic an operator safety circuit
    ...
    press at the same time to activate the machine.
    
    Replace the momentary button program blocks added above with the new
    program block, below. The program combines two if conditions to light
    LED4 (representing our machine) only when both SW4 and SW5 are pressed.
  ...
  if(SW4State == LOW) {
    ...
    program matters? Try swapping the conditional input checks for SW4 and
    SW5 and test it again if you are unsure.

8.  As shown by the example above, nesting conditional statements does work.
    But, there is an easier and shorter way to accomplish the same task by
    implementing the logical AND conditional operator represented by two
    ampersands '&&' in C. This conditional operation is the programming
    equivalent of an AND gate logic circuit. Replace the code added above
    with this new code block that implements the logical AND operator:
...
9.  If a logical AND operator can implemented in a conditional statement,
    it seems reasonable to expect that a logical OR operator must also exist.
    In C language programs, the logical OR operator is represented by double
    vertical bars '||'. Replace the double ampersand '&&' with double bars
    to make your program implement a logical OR condition. Your code block
...
10. The equivalent of the XOR logical operation is inequality -- the output
    ...
    Inequality in C programs is represented by the '!=' (not equal to)
...
2.  Microsecond-length time pulses are too short to be seen with our eyes but
  ...
  if(SW5State == LOW) {
    ...
    instead of the same 568us high delay the program was indended to match!
    The simplest way to fix the program is to comment out the 10ms delay at
    the end of the program loop with two slashes, as shown below:

  // delay(10);

    Run the program again and press SW5. Does it sound more like a tone now?
    Try altering both of the delay values, always keeping them the same. Does
    the pitch of the tone increase or decrease when the delay values are made
    smaller? Does the opposite happen if the delay values are made larger?
...
3.  Arduino has a built-in tone() function that can be used to make a specified
  ...
  if(SW5State == LOW) {
    ...
    The tone can be silenced by pressing the RESET button (SW1) on ARPS2, or
    ...
    that it plays a tone while SW5 is held and stops the tone using the
    'noTone' function when SW5 is released.
...
4.  The setup() function includes a tone() function that makes a sound on
    ...
    you can find out more about tone in the Arduino reference documents at:
...
5.  Create a program that uses each button to either generate a unique flashing
    ...
    Test each of your flashing or sound patterns to verify that they work
    as expected. Describe what happens when multiple buttons are held. Do all
    of the patterns flash the LEDs, or play the tones, at the same time, or
    does each pattern play in sequence? Explain why this is the case.
...
6.  Create a new program that uses individual if structures to simulate the
    ...
    released. Use SW5 as the Stop button to turn LED D5 off when it is
    pressed. Test your program to ensure that your simulated machine turns
    on and off as expected.
    
7.  Run the Start-Stop program created in the previous step again, and
    describe the result when both pushbuttons SW4 and SW5 are held. Does
    LED D5 turn on at all when both buttons are pressed? Would this be 
    considered safe behaviour?
    
    Let's do some additional program analysis to try to determine what's
    happening. If LED D5 turns on when both buttons are pressed, is its
    brightness the same as when just SW4 is pressed? If the brightness is
    different, can you explain what might be causing it to be different?
...
8.  As you might imagine, an industrial machine that is able to turn on even
    while its Stop button is pressed is a significant safety hazard. Use one
    or more of the logical conditional operators introduced in the program
    analysis activities, above, to make your program operate safely. The
    machine should not be able to turn on if the Stop button is being pressed.

9.  Imagine that you would like to apply your newly-acquired microcontroller
    hardware and programming experience to create a turn signal circuit for
    your bicycle. Assume that you have designed a 3D-printed holder that holds
    an Arduino, a battery pack, and up to four LEDs, and that it can easily be
    mounted on the bicycle's seat post. The LEDs will be controlled by two
    momentary pushbuttons mounted on the bicycle's handlebars. Plan the best
    way to implement easy to use and easily visible turn signals, and then
    write a program that uses the LEDs and pushbuttons on ARPS2 to simulate
    their operation.
...
10. Modify the bicycle turn signal program, above, to add one or more extra
    ...
    flashing visibility light that can be controlled using the unused buttons.
    Test your program to ensure that its functions work as expected under all
    operating conditions (e.g. signalling while braking, or using signalling
    while using the bell).
*/